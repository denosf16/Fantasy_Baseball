...
from src.visuals.batted_ball_profile_report import main as batted_ball_profile_report_main
...
def generate_reports(entity_type: str, time_grain: str, value: int) -> None:
    ...
    try:
        for idx, (_, row) in enumerate(entities.iterrows(), start=1):
            ...
            print(
                f"[{idx}/{total_entities}] Generating batted ball profile: "
                f"{entity_name} ({entity_id})"
            )

            try:
                sys.argv = [
                    "batted_ball_profile_report.py",
                    entity_type,
                    time_grain,
                    str(entity_id),
                    str(value),
                ]
                batted_ball_profile_report_main()
                success_count += 1
...
def main() -> None:
    if len(sys.argv) >= 4:
        ...
        print("\nUsage:")
        print(
            "python -m src.generators.generate_batted_ball_profile_reports "
            "<entity_type> <time_grain> <value>\n"
        )
        print("Examples:")
        print("python -m src.generators.generate_batted_ball_profile_reports hitter game 778496")
        print("python -m src.generators.generate_batted_ball_profile_reports pitcher season 2025\n")
        return
    ...
    print(
        f"\nGenerating batted ball profile reports for "
