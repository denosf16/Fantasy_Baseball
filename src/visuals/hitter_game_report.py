...
def draw_spray_chart(ax, df_bip: pd.DataFrame) -> None:
    ...
    # simple infield / outfield guide
    infield_arc = Arc((0, 0), 180, 180, theta1=45, theta2=135, linewidth=1.2, color="black")
    outfield_arc = Arc((0, 0), 320, 320, theta1=45, theta2=135, linewidth=1.2, color="black", alpha=0.6)
    ...
    if not df_bip.empty:
        if {"hc_x", "hc_y"}.issubset(df_bip.columns) and df_bip["hc_x"].notna().any() and df_bip["hc_y"].notna().any():
            x = df_bip["hc_x"]
            y = df_bip["hc_y"]
            ax.scatter(x, y, s=70, alpha=0.75, edgecolors="black", linewidths=0.3)
        elif {"hit_distance_sc"}.issubset(df_bip.columns) and df_bip["hit_distance_sc"].notna().any():
            temp = df_bip.copy()
            n = len(temp)
            temp["plot_x"] = pd.Series(range(n)).apply(lambda i: (-1) ** i * (20 + (i % 8) * 10))
            temp["plot_y"] = temp["hit_distance_sc"].fillna(0)
            ax.scatter(temp["plot_x"], temp["plot_y"], s=70, alpha=0.75, edgecolors="black", linewidths=0.3)
        else:
            ax.text(0.5, 0.5, "No spray coordinates available", transform=ax.transAxes, ha="center", va="center")
...
def build_pitch_summary(df: pd.DataFrame) -> pd.DataFrame:
    ...
    return summary.sort_values(["pitches_seen", "pitch_type"], ascending=[False, True]).reset_index(drop=True)
...
def main():
    ...
    batter_name = df["batter_name"].dropna().iloc[0] if "batter_name" in df.columns and df["batter_name"].notna().any() else str(batter_id)
    ...
    if not bip_df.empty and bip_df["launch_speed"].notna().any() and bip_df["launch_angle"].notna().any():
        ax_evla.scatter(
            bip_df["launch_angle"],
            bip_df["launch_speed"],
            s=80,
        ...
        ax_evla.text(0.5, 0.5, "No batted-ball EV/LA data", transform=ax_evla.transAxes, ha="center", va="center")
    ...
    # 4) Zone result profile
    zone_summary = pd.DataFrame(
        {
            "Metric": ["Pitches Seen", "Swings", "Whiffs", "Called Strikes", "Balls in Play"],
            "Value": [total_pitches_seen, swings, whiffs, called_strikes, balls_in_play],
        }
    )
    ax_zone.axis("off")
    zone_table = ax_zone.table(
        cellText=zone_summary.values,
        colLabels=zone_summary.columns,
        loc="center",
        cellLoc="center",
    )
    ...
    col_labels = ["Type", "# Seen", "Avg Velo", "Avg EV", "Avg LA", "Swing%", "Whiff%", "Called K%", "In Play%"]
    table = ax_tbl.table(
    ...
    fig.subplots_adjust(top=0.88, bottom=0.05, left=0.05, right=0.98, hspace=0.35, wspace=0.20)
